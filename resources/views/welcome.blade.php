<x-public-layout title="SentePro | Collect Payments. Settle Faster. Grow Your Business.">
    ...
    <section class="lg:grid lg:min-h-[32rem] lg:grid-cols-2 lg:items-stretch">
        <div class="px-4 py-12 sm:px-6 lg:flex lg:flex-col lg:justify-center lg:px-16 lg:py-16">
            ...
            <h1 class="max-w-2xl text-[length:var(--sh-mobile)] font-black tracking-tight text-slate-900 lg:text-[length:var(--sh-desktop)]" style="--sh-mobile: {{ $content->headingSize('hero') }}; --sh-desktop: {{ $content->section_heading_size_px }}px;">{{ $content->hero_headline }}</h1>
            <p class="mt-5 max-w-2xl text-lg text-slate-600 lg:text-[length:var(--sd-desktop)]" style="--sd-desktop: {{ $content->section_description_size_px }}px;">{{ $content->hero_subtext }}</p>
            <div class="mt-8 flex flex-wrap gap-2 sm:gap-3">
                <a href="{{ route('business.register') }}" class="flex-1 whitespace-nowrap rounded-full bg-lime-400 px-3 py-2.5 text-center text-[11px] font-semibold tracking-tight text-slate-950 shadow-lg shadow-lime-500/15 hover:bg-lime-300 sm:px-5 sm:py-3 sm:text-base sm:tracking-normal">{{ $content->hero_cta_text }}</a>
                <a href="/login" class="flex-1 whitespace-nowrap rounded-full border border-slate-300 px-3 py-2.5 text-center text-[11px] font-semibold tracking-tight text-slate-700 hover:border-slate-400 hover:bg-slate-50 sm:px-5 sm:py-3 sm:text-base sm:tracking-normal">Log in to dashboard</a>
            </div>
        </div>

        <div class="mt-8 px-4 sm:px-6 lg:mt-0 lg:flex lg:h-full lg:items-center lg:justify-center lg:px-0">
            <div class="overflow-hidden rounded-[2rem] border border-slate-800 bg-slate-900 shadow-xl shadow-slate-900/10 lg:h-3/4 lg:w-3/4">
    ...
    <div class="mx-auto mt-[1cm] max-w-6xl px-4 pb-10 sm:px-6 lg:px-8" id="requirements">
        <div class="max-w-2xl">
            ...
            <p class="mt-2 text-slate-600">{{ $content->requirements_subtext }}</p>
        </div>
    </div>
    <div class="space-y-[1cm]">
        @foreach (($content->requirements ?? []) as $requirement)
            @php
                ...
                $pastelBg = ['bg-orange-50', 'bg-violet-50', 'bg-lime-50'][($loop->iteration - 1) % 3];
            @endphp
            <section class="lg:grid lg:min-h-[20rem] lg:grid-cols-2 lg:items-stretch lg:gap-8">
                <div class="{{ $imageFirst ? 'lg:order-1' : 'lg:order-2' }} px-4 sm:px-6 lg:flex lg:h-full lg:items-center lg:justify-center lg:px-0">
                    <div class="overflow-hidden rounded-[2rem] border border-slate-800 bg-slate-900 lg:h-full lg:w-full">
                ...
                <div class="{{ $imageFirst ? 'lg:order-2' : 'lg:order-1' }} {{ $pastelBg }} mx-4 mt-6 rounded-[2rem] px-4 py-8 sm:mx-6 sm:px-6 lg:mx-0 lg:mt-0 lg:flex lg:flex-col lg:justify-center lg:px-16 lg:py-0">
                    <h3 class="font-semibold text-slate-900 lg:text-[length:var(--sh-desktop)]" style="--sh-desktop: {{ $content->section_heading_size_px }}px;">{{ $requirement['title'] }}</h3>
                    <p class="mt-1 text-slate-600 lg:text-[length:var(--sd-desktop)]" style="--sd-desktop: {{ $content->section_description_size_px }}px;">{{ $requirement['description'] }}</p>
                    <a href="{{ $registerUrl }}" class="mt-6 inline-flex rounded-full bg-lime-400 px-5 py-2.5 text-sm font-semibold text-slate-950 hover:bg-lime-300 lg:self-start">{{ $typeLabel ? 'Register as '.$typeLabel : 'Register now' }}</a>
                </div>
            </section>
        @endforeach
    </div>
    ...
    <div class="mx-auto mt-[1cm] max-w-6xl space-y-[1cm] px-4 sm:px-6 lg:px-8">
        ...
        <section id="features">
            <div class="mb-8 max-w-2xl">
                ...
                <p class="mt-2 text-slate-600">{{ $content->features_subtext }}</p>
            </div>
            <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
                @foreach ($content->features as $feature)
                    @php $pastelBg = ['bg-orange-50', 'bg-violet-50', 'bg-lime-50'][$loop->index % 3]; @endphp
                    <div class="{{ $pastelBg }} rounded-[2rem] p-6 shadow-sm ring-1 ring-slate-900/5">
        ...
        <section class="grid gap-10 lg:grid-cols-2 lg:items-center">
            <div>
                ...
                <p class="mt-3 text-slate-600">{{ $content->balances_subtext }}</p>
            ...
            <div class="rounded-[2rem] bg-white p-6 shadow-sm ring-1 ring-slate-900/5">
                <div class="space-y-3">
                    <div class="flex items-center justify-between rounded-xl bg-slate-50 px-4 py-3">
                        <span class="text-sm text-slate-600">Available balance</span>
                        <span class="text-sm font-semibold text-slate-900">Ready to settle</span>
                    </div>
                    <div class="flex items-center justify-between rounded-xl bg-slate-50 px-4 py-3">
                        <span class="text-sm text-slate-600">Pending balance</span>
                        <span class="text-sm font-semibold text-slate-900">Awaiting settlement</span>
                    </div>
                    <div class="flex items-center justify-between rounded-xl bg-slate-50 px-4 py-3">
                        <span class="text-sm text-slate-600">Settlement balance</span>
                        <span class="text-sm font-semibold text-slate-900">Paid out</span>
                    </div>
                </div>
            </div>
        </section>
    ...
    <div class="mx-auto mt-[1cm] max-w-6xl px-4 sm:px-6 lg:px-8">
        <section class="overflow-hidden rounded-[2rem] lg:grid lg:min-h-[30rem] lg:grid-cols-2 lg:items-stretch">
            <div class="order-2 bg-slate-900 px-4 py-16 sm:px-6 lg:order-1 lg:flex lg:flex-col lg:justify-center lg:px-16 lg:py-0">
                ...
                <a href="{{ route('business.register') }}" class="mt-6 inline-flex rounded-full border border-white/15 px-5 py-3 font-semibold text-white hover:border-white/30 hover:bg-white/5 lg:self-start">Get started</a>
            </div>
    ...
    {{-- How it works --}}
    <div class="mx-auto mt-[1cm] max-w-6xl px-4 sm:px-6 lg:px-8">
        <section id="how-it-works" class="overflow-hidden rounded-[2rem] lg:grid lg:min-h-[30rem] lg:grid-cols-2 lg:items-stretch">
        ...
        <div class="bg-slate-900 mt-6 px-4 py-10 sm:px-6 lg:mt-0 lg:flex lg:flex-col lg:justify-center lg:px-16 lg:py-0">
            <h2 class="text-[length:var(--sh-mobile)] font-bold text-white lg:text-[length:var(--sh-desktop)]" style="--sh-mobile: {{ $content->headingSize('how_it_works') }}; --sh-desktop: {{ $content->section_heading_size_px }}px;">{{ $content->how_it_works_heading }}</h2>
            ...
            <a href="{{ route('business.register') }}" class="mt-6 inline-flex rounded-full bg-lime-400 px-5 py-3 font-semibold text-slate-950 hover:bg-lime-300 lg:self-start">{{ $content->how_it_works_cta_text }}</a>
        </div>
        </section>
    </div>
    ...
    {{-- CTA banner --}}
    <div class="mx-auto mt-[1cm] max-w-6xl px-4 sm:px-6 lg:px-8">
        <section class="relative overflow-hidden rounded-[2rem] bg-slate-900 py-20">
            ...
            <div class="relative mx-auto flex max-w-6xl flex-col items-center gap-6 px-4 text-center sm:px-6 lg:px-8">
                ...
                <a href="{{ route('business.register') }}" class="rounded-full bg-lime-400 px-6 py-3 font-semibold text-slate-950 hover:bg-lime-300">Register now</a>
            </div>
        </section>
    </div>
</x-public-layout>
